...
class ICLR:
    def __init__(self, year):
        self.url = f"https://iclr.cc/static/virtual/data/iclr-{year}-orals-posters.json"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Fetch ICLR GAD papers from official API")
    parser.add_argument("--year", default=config["default_year"],
                        help=f"Conference year (default: {config['default_year']})")
    args = parser.parse_args()

    iclr = ICLR(year=args.year)
    res = iclr.do_requests()
